...
- The `Class & Spells` phase now has a local `Classes | Spells` subview switch so players can open spell selection without scrolling past every class decision.
- Inside each selected class, subclass, skill-proficiency, ASI/Feat, and feature-choice sections now collapse independently and default open when unresolved `Fix` issues are present.
